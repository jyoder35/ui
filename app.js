...
function showStatus(text, type=""){ const s = $("statusLine"); s.textContent = text; s.className = "status" + (type ? (" " + type) : ""); setFooterIndicator(text, type); }
...
/* ===== NEW: WS4 Logger endpoint (paste your deployed Web App URL below) ===== */
const WS4_LOG = ""; // e.g., "https://script.google.com/macros/s/XXXXX/exec"
...
async function logRunToQuotes({ inputs, quoteWith, quotePar }) {
  try {
    if (!WS4_LOG) return; // gracefully no-op until you paste the Web App URL
    const leadToken = localStorage.getItem("azm_leadToken") || "";

    // Gather minimal lead meta if you want better Leads tab (optional, we send blanks if absent)
    const leadMeta = {
    ...
    // Dev inspector
    captureDev("devSaveReq", payload); // reuse the right-hand dev pane for visibility

    // Fire-and-forget pattern: no blocking UI, but we still read the response to surface errors in dev mode
    ...
    captureDev("devSaveRes", text, 2000);
    // We do not toast on success to avoid noisy UX. If it fails, we also stay silent (dev pane shows details).
  } catch (_e) {
    // Silent catch; devs can watch the inspector if needed.
  }
}
